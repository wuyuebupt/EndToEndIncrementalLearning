...
imdb_path = '/local1/incremental_cvpr2018/faceNew/eeil/imagenetMatlab/imdb.mat'; % Edit me!
outdir = '/local1/incremental_cvpr2018/faceNew/eeil/imagenetMatlab/imdb/'; % Edit me!

%% Build base imdb

imdb_ = load(imdb_path);

% clsses_  = imdb.classes;
% imageDir_ = imdb.imageDir;
% images_   = imdb.images;
...
for nit=1:max_iters
    ...
    for i=1:length(batch_sizes) % Batch_sizes
        ...
        for j=1:nParts
            ...
            positions = find(ismember(imdb_.images.label, classes));
            %% Build imdb.images
            
            imdb.classes = imdb_.classes;
            imdb.imageDir = imdb_.imageDir;
%             imdb.images.
%             imdb.imageDir = 

            imdb.images.id    = imdb_.images.id(positions);
            imdb.images.name  = imdb_.images.name(positions);
            imdb.images.set   = imdb_.images.set(positions);
            imdb.images.label = imdb_.images.label(positions);
