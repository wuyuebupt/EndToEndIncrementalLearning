function [net,stats] = cnn_train_dag_exemplars(net, imdb, getBatch, varargin)
...
if start >= 1
    ...
    [net_, stats] = loadState(modelPath(start)) ;
    if ~opts.distillation
        net = net_;
    else
        if opts.addStat
            for ii = 1 : 2 : length(opts.derOutputs)
                if ~isfield(stats.train, opts.derOutputs{ii})
                    [stats.train.(opts.derOutputs{ii})] = deal(0);
                end
            end
        end
    end
...
for t=1:opts.batchSize:numel(subset)
    ...
    for s=1:opts.numSubBatches
        ...
        if opts.prefetch
            ...
            wk = parfeval(state.c, state.getBatch, 1, state.imdb, nextBatch);
        end
        
        if strcmp(mode, 'train')
            %            % Clean previous der.
            %            [~, ~, net] = fc_cleanOldValues(net);
            
            net.mode = 'normal' ;
            net.accumulateParamDers = (s ~= 1) ;
            net.eval(inputs, opts.derOutputs) ;
        
            stats_ = opts.extractStatsFn(net, 'dagnn.SoftmaxDiffLoss') ;
            stats__ = opts.extractStatsFn(net, 'dagnn.Loss') ;
            for fn = fieldnames(stats_)'
                stats__.(fn{1}) = stats_.(fn{1});
            end
        else
            net.mode = 'test' ;
            net.eval(inputs) ;
    ...
    if opts.plot_weights
        diffs_ = fc_computeDiffWeights(net_old, net);
        diffs = cat(3, diffs, diffs_);
    end
    ...
    for fn = fieldnames(stats__)'
        stats.(fn{1}) = stats__.(fn{1});
    end
    stats.num = num ;
    stats.time = time ;
    ...
    index = strfind({net.layers.name}, 'scale_loss');
    index = find(not(cellfun('isempty', index)));
    if ~isempty(index)
        val = net.params(net.layers(index(1)).paramIndexes(1)).value;
        fprintf(' lambdas: [%.3f', val);
        for parr_ix = 2:length(index)-1
            val = net.params(net.layers(index(parr_ix)).paramIndexes(1)).value;
            fprintf(' %.3f', val);
        end
        val = net.params(net.layers(index(end)).paramIndexes(1)).value;
        fprintf(' %.3f]', val);
    end
    fprintf('\r') ;
end
...
for p=1:numel(net.params)
    ...
    switch net.params(p).trainMethod
        ...
        case 'gradient'
            ...
            thisPenalty = sum(net.params(p).value(:) .^ 2);
            thisDecay = thisDecay * gather(thisPenalty);
            thisLR = state.learningRate * net.params(p).learningRate ;
            thisNoise = (0.3 / ((1 + state.epoch) ^ 0.55)) .* randn(size(net.params(p).der));
            
            parDer = net.params(p).der + thisNoise;
            parDer = vl_taccum(1/batchSize, parDer, ...
                double(thisDecay), net.params(p).value) ;
            ...
            net.params(p).value = vl_taccum(...
                1,  net.params(p).value, thisLR, state.momentum{p}) ;
...
% -------------------------------------------------------------------------
function stats = extractStats(net, func)
% -------------------------------------------------------------------------
sel = find(cellfun(@(x) isa(x,func), {net.layers.block})) ;
sel_ = find(cellfun(@(x) isa(x, 'dagnn.DiffLoss'), {net.layers.block})) ;
sel = cat(2, sel, sel_);
